...
public class gilde_SetHome extends JavaPlugin{
	public static ArrayList<String> boughtHome = new ArrayList<String>();
	...
	public static void execute(Integer nr) {
		...
		if(ToolBox.getallPlayers().contains(player.getUniqueId().toString())) {
			//check Rank
			if(!Main.getSaves().getBoolean("gilden." + gilde + ".Information." + "hasBoughtHome")) {
				player.sendMessage("§cUm diesen Befehl zu nutzen musst du dir zuerst das Home-Recht kaufen!");
			}else {
